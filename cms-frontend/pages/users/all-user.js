...
const AllUsers = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await fetch('https://hellocms.dinasuvadu.com:5000/api/users/all-users');
  ...
  const handleDelete = (record) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          const response = await fetch(`https://hellocms.dinasuvadu.com:5000/api/users/${record._id}`, { method: 'DELETE' });
