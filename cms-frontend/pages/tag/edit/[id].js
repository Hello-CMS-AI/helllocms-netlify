...
const EditTagPage = () => {
  ...
  const fetchTagDetails = useCallback(async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`https://hellocms.dinasuvadu.com:5000/api/tags/${id}`);
      if (response.ok) {
  ...
  const handleUpdateTag = async (values) => {
    try {
      setLoading(true);
      const response = await fetch(`https://hellocms.dinasuvadu.com:5000/api/tags/update-tag/${id}`, {
        method: 'PUT',
